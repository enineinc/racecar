...
Templates live in [`../templates/classic/`](../templates/classic/) — a single canonical set (black + isort + pylint; no ruff). The project shapes, the full file inventory (`library-pyproject.toml`, `server-pyproject.toml`, `Makefile`, `pre-commit-config.yaml`, `gitignore`), and the copy-and-substitute procedure live in [`PACKAGING.md`](PACKAGING.md). The arch-coherence-specific steps on top of that procedure are:
