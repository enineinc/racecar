...
The `django` shape is the flat `django-admin startproject` layout: `manage.py` at the repo root, apps as siblings, no library. racecar recognizes it because it is Django's own canonical output — racecar may *prefer* `server/` for a library-backed service (where the library is the architectural center, see §"Why"), but a preference over layouts must not un-recognize the framework's default (django > racecar). It differs from the library shapes in two ways the audit respects: (1) its root `pyproject.toml` is a **config-home** (tool config, `ignore-paths`), not a publishable library manifest, so the library-pyproject audit ([project]/[build-system]/dev-group) is **skipped** for it — the same reasoning as a repo with no `[project]` not being a package (§8 / VERSION); its version home is a root `VERSION` file. (2) `SRC` defaults to `.` (the whole repo root is the tree); narrow it to the real first-party app dirs with a `:=` override in the owned `Makefile`. A root `manage.py` still triggers the Django-specific audit and pre-commit hook, so `django` gets the same ORM-relation checks as the `server` shapes.
