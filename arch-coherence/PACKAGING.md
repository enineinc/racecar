...
A shape is the composition of two atoms — a **PYTHON_LIBRARY** (`src/<pkg>`) and a **DJANGO_PROJECT** (a `manage.py`): `src` (library only), `src+server` (library × Django under `server/`), `server` (Django under `server/`, no library), and `django` (a flat Django site — `manage.py` at the repo root, no library; the `django-admin startproject` canon). The first three are racecar's library-centric shapes (the `src/` + `server/` split argued below); `django` is the shape for a standalone Django *site* with no library to wrap, recognized because it is Django's own default output (python > racecar; django > racecar — racecar may *prefer* `server/` but does not un-recognize the framework's canon). The choice of shape is local to the project; the rest of the opinion (PEP 621 + PEP 735 pyproject as sole source of truth, `make check-full` as CI gate, the dev tool set, `.venv/` discipline, no VC-backed tooling) is identical across all of them. From the shape, `racecar.mk` sets several variables that the rest of the build reads: `SRC` (where the Python source lives), `PKG` (the importable package path used by audits), `SERVER` (the Django app directory, empty when not Django), `LIB_PYPROJECT` (path to the library pyproject), and `SERVER_PYPROJECT` (path to the server pyproject, only for Shape src+server).
...
The shape is the **product of a PYTHON_LIBRARY presence** (`src/`) **and a DJANGO_PROJECT presence** (a `manage.py`) racecar reads off the layout, over the shared **root `pyproject.toml`** every shape carries. The Django axis is further split by *where* `manage.py` lives: under `server/` (racecar's server shell) or at the repo root (the flat, `django-admin startproject` site). The shape names are the *derived labels* for the cells of that product; the enumeration is a shorthand over the product, not in lieu of it. Read "`X` exists" as "the path `X` is present in the repo":

| PYTHON_LIBRARY (`src/`) | DJANGO_PROJECT (`manage.py`) | Shape |
|---|---|---|
| ✓ | — | **`src`** — library only |
| ✓ | `server/manage.py` | **`src+server`** — library × Django |
| — | `server/manage.py` | **`server`** — racecar's server-shell Django, no library |
| — | `manage.py` (repo root) | **`django`** — flat Django site (`startproject` canon), no library |
| — | — | **neither** — a no-shape finding (`unknown` in Python, `stock` in Make), neutral defaults (§7) |

The `(—, —)` cell is a real cell, not a fallback to `src`: a root `pyproject.toml` with none of a `src/` library, a `server/manage.py`, or a root `manage.py` is reported as "not a recognized shape," never silently classified as a library. Without a root `pyproject.toml` at all, the repo is likewise unclassifiable (the same cell). Django is marked by a `manage.py`, never a bare `server/` directory: a `server/` holding only a pyproject (no `manage.py`) is not a runnable Django app, so it does not make the project a server. A root `manage.py` beside a `src/` library is **not** the flat `django` shape — a library's Django belongs under `server/` (the `src+server` convention), so that case degrades to the library reading (`src`), it does not mint a new cell. **Multi-library support is trigger-gated, not built.** The `{packages,pypkg}/<pkg>/src/<pkg>` workspace form (multiple co-versioned libraries in one repo) ships the moment a single repo genuinely holds a second co-versioned library, and not before: an abstraction earns its place at its second real consumer, never on speculation. That trigger is deliberately unmet today. Single-library-per-repo is the current design stance, an intentional choice and not a backlog item, so the grouping container has no instance to serve yet. When the trigger fires, one governance question comes with it: `packages/` is uv/pnpm/Pants workspace convention, not PSF/PyPA canon, and whether a bare directory name (no dependency, no config block, trivially renamable) trips the no-VC-tooling rule (§1) or that rule reaches only tool *dependencies and lock-in* is a real interpretation call. Settle it when the skill is built; name the skill for the act (grouping), not the container, so the container name stays a swappable detail.
...
| Shape | PYTHON_LIBRARY (`src/<pkg>`) | DJANGO_PROJECT (`manage.py`) | Library pyproject | server pyproject | `SRC` | `PKG` | `SERVER` |
|---|---|---|---|---|---|---|---|
| **`src`** | ✓ | — | `pyproject.toml` (root) | — | `src` | `src/<pkg>` | *(unset)* |
| **`src+server`** | ✓ | `server/` | `pyproject.toml` (root) | `server/pyproject.toml` | `src` | `src/<pkg>` | `server` |
| **`server`** | — | `server/` | `pyproject.toml` (root) | — | `server` | `server/<app>` | `server` |
| **`django`** | — | root | `pyproject.toml` (root, config-home) | — | `.` | `.` | *(unset)* |

The `server` shape is a standalone Django project under `server/` (its `manage.py`), with the library pyproject at the repo root and `SRC=server`; narrow `PKG` to the actual Django package directories with a `:=` override in the owned `Makefile` if needed.

The `django` shape is the flat `django-admin startproject` layout: `manage.py` at the repo root, apps as siblings, no library. racecar recognizes it because it is Django's own canonical output — racecar may *prefer* `server/` for a library-backed service (where the library is the architectural center, see §"Why"), but a preference over layouts must not un-recognize the framework's default (django > racecar). It is a **config-home site, not a package**, and the audit exempts it from the package-only canon that has nothing to act on there:

- Its root `pyproject.toml` is a config-home (tool config, `ignore-paths`), not a publishable library manifest, so the library-pyproject audit (`[project]`/`[build-system]`/dev-group/`[tool.*]` canon) is **skipped** — the same reasoning as a repo with no `[project]` not being a package (§8 / VERSION). Its version home is a root `VERSION` file.
- A standalone `.pylintrc` is **allowed** (not the `standalone-pylintrc` Blocker): pylint natively supports both `.pylintrc` and pyproject, and `.pylintrc` + pylint-django is the idiomatic Django config home. Consolidating pylint config into pyproject is a racecar *preference* for packages, not a rule to force onto a site.
- The package-only pre-commit hooks (`import-linter`, `validate-pyproject`, `no-upward-imports-in-business-modules`) are **not required**: a config-home has no import-linter contracts, no `[project]` to validate, and no src-package upward-imports. The shape-independent hooks (format, doc-coherence, TODO, placement) stay required.

Two positive knobs remain: `SRC` defaults to `.` (the whole repo root); narrow it to the real first-party app dirs with a `:=` override in the owned `Makefile`. And a root `manage.py` still triggers the Django-specific audit and pre-commit hook, so `django` gets the same ORM-relation checks as the `server` shapes.
...
- For the Django shapes (`src+server`, `server`, `django`): a `manage.py` — under `server/`, or at the repo root for `django` — triggers the Django-specific audit (`scripts/check_dj_model_ref_as_string.py`) and pre-commit hook automatically.
...
This opinion does not accommodate everything. uv-shops, Bazel monorepos, src-on-top layouts without `src/`, and pyproject-at-package-root layouts other than `src/` are deliberately not supported. A project adopting this opinion picks one of the four valid shapes (`src` / `src+server` / `server` / `django`, the populated cells of the product) and commits — the first three when there is a library to package, `django` for a standalone Django site with none.
...
- **Disable set is fixed.** A project may *append* its own disables, but every entry in the canonical `[tool.pylint."MESSAGES CONTROL"].disable` must be present: the pre-commit-plumbing noise codes, plus `duplicate-code` (R0801 fires on argparse / pydantic / dataclass boilerplate), `too-few-public-methods` (R0903 fires on pydantic models, config / exception classes, and django middleware), and the two `use-implicit-booleaness` idioms.
