...
**Unidirectional relationship.** The dependency graph is parent → child only. Parents know about their children (by registering their names). Children never reference parents. This is the CLI-layer expression of the direction axiom in [check 2 Direction](CHECKS.md#2-direction).
...
Note: `_print_commands()` must exist in every Pattern 1 and Pattern 2 `__main__.py`. The outward-downward rule ([check 2 Direction](CHECKS.md#2-direction)) forbids inheriting it from above; there is no guaranteed leaf below to reuse it from. The function is therefore explicit at every node — a direct expression of the architecture. Its *body*, however, is identical boilerplate, and that part may be shared.
...
| No inward references in `__main__.py` — no `from ..` | Prevents upward references that would couple children to parents (see [check 2 Direction](CHECKS.md#2-direction)) and mask circular deps (see [check 1 Acyclicity](CHECKS.md#1-acyclicity-root-axiom)) |
