...
### Added
- **`racecar-upgrade` now instruments every compliant CLI for usage telemetry — mechanically.** Build telemetry rode the `make` gate (which sync owns); usage telemetry instruments the package's own `python -m <pkg>` entrypoints, which sync couldn't — so it never rolled out. [`scripts/instrument_telemetry.py`](scripts/instrument_telemetry.py) closes that with no LLM judgment: it delivers the probe to each top package and `ast`-wraps each `__main__.py` run-guard, editing **only** guards it can prove are the `main()` / `sys.exit(main())` / `raise SystemExit(main())` form (reading the callable name from the AST, splicing just those lines), and **surfacing** anything else for manual handling. Idempotent; `--check` mode reports without writing. Wired into the upgrade flow; covered by `test_instrument_telemetry.py`.
- **Fixed: `run()` discarded the entrypoint's exit code.** The probe's `run(main)` was `with record(): main()` — it threw away `main()`'s return, forcing every wrapped CLI to exit 0. Now `run(main)` is `sys.exit(main())` with measurement: the return (or an explicit `sys.exit`) propagates as the exit code, recorded and re-raised. This had to land before auto-wrapping, or instrumentation would have broken exit codes fleet-wide (CI, `&&`, pre-commit). Covered in `test_telemetry.py`.
- **The build-telemetry harvest: the feedback loop that flows the gate ledger back to racecar.** Build telemetry measures racecar's own checkers, so — unlike usage telemetry, which stays with the repo it measures — it only earns its keep by flowing *back*. `scripts/harvest_build_telemetry.py` pulls each target's `.telemetry/build.jsonl` into an anonymized fleet aggregate (per-writer files under `.telemetry/fleet/`, laid out to map 1:1 onto a shared `racecar-telemetry` git repo — the transport is designed but deferred), and `scripts/fleet_profile.py` reduces the aggregate to which checkers are **dead** fleet-wide (kill candidates), which are **noisy** (over-broad), and whether a release moved findings (trend by canon version). This is the proving-ground direction (project → racecar) made continuous. `make harvest DEST=<repo>` and `make fleet` wrap the two; `racecar-upgrade` harvests a repo before syncing it. Covered by `test_harvest_build_telemetry.py` / `test_fleet_profile.py`.
- **Two walls, enforced by construction.** *Company wall*: only structural checker data crosses up, keyed to a machine-independent, pseudonymous `repo_id` (the hashed git root-commit SHA — identical on every clone, so one repo is one id everywhere); no real SHA, branch, command, or path leaves the target. *Person wall*: no writer identity and no timestamp is ever in a record (`writer_id` names the file only, for conflict-free merge and diversity counts), so the aggregate can profile a checker but never a person. The shared record is minimized to `{racecar_version, checker, fired, findings}` — harmless by design under an opt-out default.
- **k-anonymity in the reducer.** `fleet_profile.py --k N` (default 5) shows a checker only when at least N *distinct repos* contributed it; below that cohort a stat could point back at one repo (since `repo_id` is pseudonymous, not anonymous), so it is withheld and only counted in the summary — never silently dropped. `--k 1` is the aggregator's own full local view; a higher `--k` hardens a report meant to be shared. The same defense gstack applies at its community read boundary.
- **Collect now, push later — every record carries a `pushed` flag.** Collection is live; the transport to a shared sink is a deliberately deferred decision (S3 drop-bucket vs. a small write-API + database — each fits different requirements). To keep that clean, every build record is written with a stable `id` and `pushed: false`. When the push lands it will send only `pushed == false` records (anonymized at source via `anonymize()`), then flip them to `true` keyed on `id` — so the local log always shows, git-style, which records have gone to the fleet and which have not. No double-sends, no ambiguity, and the fleet-wide gate rollout is held until the transport is chosen.
- **The telemetry directory self-ignores — the ledger can never be committed.** The first write drops a `.telemetry/.gitignore` containing `*` (the pytest-`.pytest_cache` pattern, which git honors tracked or not), so `.telemetry/` stays out of every commit in *every* repo regardless of its root `.gitignore` — closing the gap where an existing adopter (whose create-if-missing `.gitignore` never received a `.telemetry/` line) could have committed the raw local ledger. All three writers (`record_gate.py`, the usage probe, `telemetry_toggle.py`) ensure it, best-effort; the canonical `gitignore` template also lists `.telemetry/` for visibility.
- **Consent by disclosure + two toggles.** A SessionStart hook (`hooks/session_telemetry_notice.py`) prints `racecar telemetry: build=… share=… usage=…` every session entry in a racecar repo, so on-by-default is never silent. `/racecar-telemetry-build [on|off]` and `/racecar-telemetry-share [on|off]` (backed by `scripts/telemetry_toggle.py`, delivered to adopters) write a per-developer, gitignored `.telemetry/settings.toml`, resolved **env > settings.toml > pyproject > on** by both `record_gate.py` and the usage probe. A new `share` switch (`RACECAR_SHARE_TELEMETRY`) gates whether the anonymized aggregate may leave the machine.
- **`record_gate.py` is delivered and wired.** It is now in `CHECK_SCRIPTS` (synced to adopters), and the canonical `check` / `arch` in `templates/classic/racecar.mk` — and racecar's own `Makefile` — route their body through it (guarded: a missing wrapper never breaks the gate), so every governed repo emits build telemetry automatically. Each record now carries `racecar_version` (the `scripts/.racecar-version` stamp in force), letting the fleet attribute a findings shift to the exact canon version; record schema **1 → 2**. `check_packaging`'s §7 Makefile rules follow the wrapper to the private `_check` / `_arch`, so the gate contract is unchanged whether recorded or direct (PACKAGING.md §7 updated to match).

## 0.28.0 - 2026-07-19

### Added
- **Telemetry schema 2: a `mark()` work-scale hook and a run-time `provenance` block.** `sysadmin-hardware/lib/_telemetry.py` now captures two backward-only signals — knowable only at run time, lost afterward, useful in accumulation. `mark(rows=…, bytes=…, files=…)` lets a command self-report how much work it did; the counters accumulate across calls into the record's `work` object, turning every resource number into a *rate* (per-row, per-MB) so sizing can project to future scale instead of only fitting past peaks. `provenance` records the run-time git SHA + dirty flag, Python version, host, virtualenv, and a hash of the installed dependency set — gathered *after* the resource snapshots so it never inflates the measurement — attributing a later resource or failure shift to the exact code and environment that produced it. `mark()` is a no-op when telemetry is off and never raises; `TELEMETRY.md` documents both and `test_telemetry.py` covers them. The reducer reads fields by name, so the added keys don't disturb it.
- **Argv secret redaction.** The probe now masks secret-shaped tokens (the value after a secret-named flag like `--token`/`--password`, standalone key shapes — JWT, `ghp_`/`sk-`/`AKIA…`, PEM headers — and URL passwords) to `<redacted>` before the record is written. It closes the privacy gap TELEMETRY.md already named (argv was stored verbatim) and, because masking happens once up front, prevents a stray credential from leaking into the derived `subcommand`/`command` fields too. Stdlib-only, conservative (paths and ordinary args pass through). The redaction posture is borrowed from gstack's pre-log redaction discipline; the implementation is racecar's own stdlib version.
- **Two telemetry streams, split cleanly and configured in one home.** racecar now distinguishes **usage** telemetry (the runtime resource probe — what a command costs, for the sizing lens) from **build/developer** telemetry (gate outcomes over time — the drift trajectory). They never share a switch or a file: usage writes `.telemetry/usage.jsonl` behind `RACECAR_USAGE_TELEMETRY`, build writes `.telemetry/build.jsonl` behind `RACECAR_BUILD_TELEMETRY`. Both are **on by default (opt-out)** — opt-in telemetry mostly never gets turned on, so the data never accumulates; it is safe to default on because the log is local, gitignored, argv-redacted, and the probe never alters a command — resolved env > `[tool.racecar.telemetry]` in pyproject (`usage`/`build`/`dir`) > on, with a shared `RACECAR_TELEMETRY_DIR`. Opt out with `usage`/`build = false` or `RACECAR_*=0`. `check_racecar_overrides` allowlists `[tool.racecar.telemetry]` as a legitimate binding, and racecar opts itself in via its own pyproject. **Breaking:** the probe's switch is renamed `RACECAR_TELEMETRY` → `RACECAR_USAGE_TELEMETRY`; export the new name.
- **Build telemetry: a gate-outcome ledger (`scripts/record_gate.py`).** A transparent wrapper — `record_gate.py <label> -- <command>` — runs a gate (`make check`, `make arch`), streams output through, and appends one record keyed to the commit: `{git_sha, git_dirty, branch, ok, exit_code, wall_s, total_findings, checkers}` (findings parsed best-effort from the `<name>: OK` / `<name>: N errors` convention; exit code authoritative). Accumulated across commits it is the DRIFT trajectory (`shared/DRIFT.md`'s "the ledger") — which checkers recur, whether findings trend to zero — the backward-only signal a single green run can't show. Covered by `test_record_gate.py`.
...
- `make clean` now delegates to the canonical `scripts/clean_files.sh` — the single home racecar ships to adopters via `templates/classic/racecar.mk` — so racecar's own clean cannot drift from the clean it hands out. `distclean` still layers the venv removal on top; no behavior change.
...
- **CLI telemetry, the empirical half of the lens.** `sysadmin-hardware/lib/_telemetry.py` is an optional, stdlib-only runtime probe copied into a governed repo as `<pkg>/_telemetry.py` (the same offered-template shape as `arch-coherence/lib/_cli.py`). It attaches at the one `main()` dispatch seam per `__main__.py` with a one-line wrap (`run(main)`), covering every subcommand uniformly, and appends one JSON resource record per invocation: command, argv, wall-clock, CPU time, cores actually used (`cpu_total/wall`), peak RSS, exit status, worker count, and CPU count. Off by default (opt in via `RACECAR_USAGE_TELEMETRY=1`, consistent with dotenv-at-entrypoints); never changes command behavior or output; failures are swallowed. Storage is append-only JSONL at `./.telemetry/usage.jsonl`. The reader `sysadmin-hardware/scripts/telemetry_profile.py` reduces the log to a per-command p50/p95/max profile sorted by peak RSS. The schema, hook point, enable switch, and adoption are documented in `sysadmin-hardware/TELEMETRY.md`.
