...
def render_block(racecar_root: Path) -> str:
    ...
    return (
        ...
        f"Load a skill/lens ON DEMAND when the task matches, never speculatively:\n"
        f"- scaffold a new src/<pkg> package                  -> /racecar-create-package\n"
        f"- scaffold a vanilla Django project (server shell)  -> /racecar-start-django-project\n"
        f"- expose the CLI as a REST API + MCP server         -> /racecar-create-server\n"
        f"- close the surfaces with auth (OAuth 2.1 + FIDO2)  -> /racecar-secure-server\n"
        f"- ship the server to a host (TODO)                  -> /racecar-deploy-server\n"
        f"- architecture / import cycles / layers / surfaces  -> /racecar-arch-coherence\n"
