...
A first concrete instance ships as [`scripts/record_gate.py`](../scripts/record_gate.py): a transparent wrapper around a gate run (`make check`, `make arch`) that appends one outcome record — commit SHA, pass/fail, per-checker finding counts, the canon version in force — to `.telemetry/build.jsonl`, on by default (opt-out) behind `RACECAR_BUILD_TELEMETRY` (or `[tool.racecar.telemetry].build`). This is **developer telemetry** — the trajectory of the codebase's own health — and is kept strictly separate from the **usage telemetry** (`RACECAR_USAGE_TELEMETRY`, `.telemetry/`; [`sysadmin-hardware/TELEMETRY.md`](../sysadmin-hardware/TELEMETRY.md)) the sizing lens collects about runtime cost: different question, different producer, different switch, different store. Accumulated across commits it is the clock's tick — which checkers recur, whether findings trend toward zero — the direction a single green run cannot show.

The ledger's subject is racecar's own checkers, so — unlike usage telemetry, which stays with the repo it measures — it earns its keep only by flowing **back** to racecar. [`scripts/harvest_build_telemetry.py`](../scripts/harvest_build_telemetry.py) pulls each target's `build.jsonl` up into a fleet aggregate (per-writer files under `.telemetry/fleet/`, destined for a shared `racecar-telemetry` repo); [`scripts/fleet_profile.py`](../scripts/fleet_profile.py) reduces it to which checkers are **dead** fleet-wide (kill candidates), which are **noisy** (over-broad), and whether a release moved findings (trend by canon version). This is the proving-ground direction (project → racecar) made continuous — the falsification loop as a running instrument, not a manual sweep. It is bounded by **two walls**: the *company wall* — only structural checker data crosses, keyed to a machine-independent, pseudonymous `repo_id` (the hashed git root-commit SHA); no real SHA, branch, command, or path; and the *person wall* — no writer identity and no timestamp is ever in a record, so the aggregate can profile a checker but never a person. Sharing off-machine is itself an opt-out switch (`RACECAR_SHARE_TELEMETRY`), and the telemetry state is disclosed every session — consent by disclosure plus a payload minimized to `{racecar_version, checker, fired, findings}`, harmless by design.
