...
> Load `eng-review/WORKFLOW.md`. Run the pre-pass (two checks + red flags), invoke gstack `/plan-eng-review`, then apply `eng-review/PYTHON.md` or `eng-review/DJANGO.md` as applicable. Emit one merged numbered list tagged `[pre]`/`[gstack]`/`[post]`, verdict at the end.

> Using `eng-review/WORKFLOW.md`, wrap gstack's review with racecar's Python/Django hygiene pass.
