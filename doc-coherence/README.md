...
The mechanical checks live under `scripts/`: `check_docs.py` (links, section refs, vocabulary), `check_doc_graph.py` (the `pnode` graph: targets exist, the graph is acyclic, `Accessed via` agrees with `pnode`), `check_file_placement.py` (resolver reachability: every doc must be linked from a README), `check_subsystem_docs.py`, and `check_todo_format.py`. `PROTOCOL.md` and `DOC_GRAPH.md` document what each enforces.
